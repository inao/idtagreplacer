...
public class ScriptParagraphSign extends ParagraphSign {
	...
	public ScriptParagraphSign(String startSign, String endSign, ParagraphTag tag) {
...
//		App.getInstance().getActiveParagraphTag().removeLast();
	...
	protected String whenStartSignMuches(String line) throws IOException, SourceParserException {
		...
		if (ret.equals("undefined")) ret = "";
		if (ctl.hasCommands()) return ret;
		
		if (hasEndSign()) {
//			if (ret.trim().length() != 0) App.out.println(ret);
			if (ret.trim().length() != 0) ctl.addCommand("print");
		} else {
//			App.out.println(ret);
//			App.getInstance().getActiveParagraphTag().removeLast();
			ctl.addCommand("print");
...
//		r.addPreCode("importPackage(Packages.jp.gihyo.wdp.idtagreplacer);");
		...
		if (endSign != null) r.addPreCode("var endSign = '" + endSign.replace("'", "\\'") + "';");
