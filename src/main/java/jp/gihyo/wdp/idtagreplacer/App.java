...
public class App {
	...
	/**
	 * 現在有効な段落タグを保持します。 これはリストとして実装されていますが、現状、単純なスタックのように動作しています。
	 * アプリケーションは起動時、タグ設定ファイルを読み込んだ際に、 まずこのリストに「デフォルトの段落スタイル」を登録します。
	 * その後、処理対象のテキストファイルを読み込みながら、 段落の開始記号が出現するたびに、このリストに対応する段落スタイルをpushし、
	 * 終了記号が出現するたびに、popします。
	 */
	private LinkedList<ParagraphTag> activeParagraphTag = null;

	/**
	 * 段落編集記号のリストを保持します。 タグ設定ファイルに記述された内容が、パーサを通じてここに格納されます。
	 */
	private LinkedList<ParagraphSign> paragraphSigns = null;

	/**
	 * 文字編集記号のリストを保持します。 タグ設定ファイルに記述された内容が、パーサを通じてここに格納されます。
	 */
	private LinkedList<CharacterSign> characterSigns = null;

	/**
	 * 自由置換編集記号のリストを保持します。 タグ設定ファイルに記述された内容が、パーサを通じてここに格納されます。
	 */
	...
	public void cleanupActiveParagraphTag() {
		activeParagraphTag = new LinkedList<ParagraphTag>() {
			...
			public boolean add(ParagraphTag o) {
				Logger.global.info("現在の段落タグとして、'" + o.getTagName()
						+ "'が登録されました。");
				return super.add(o);
			}
			...
			public ParagraphTag removeLast() throws NoSuchElementException {
				...
				Logger.global.info("現在の段落タグから'" + ret.getTagName()
						+ "'を取り除きました。");
				return ret;
			}
		};
	...
	/**
	 * 現在の段落スタイルをリストで返します。 <br/>
	 * このプログラムでは、段落スタイルをネスト可能にしています。
	 * これは、出現した段落スタイルをリスト（実質スタックとして利用されている）で保持することで実現しています。 このメソッドは、そのリストを返します。
	 * 
	 * @return 現在の段落スタイル
	 */
