...
class PropertiesLoader {
	...
	/**
	 * 既存の PropertiesLoader オブジェクトを返します。 既存のオブジェクトがない場合（まだ getNewInstance
	 * を呼んでいない場合）は null を返します。
	 */
...
class PropertiesLoaderHelper {
	...
	private void readTagsSettings(SimpleTagconfParser parser,
			PropertiesInfo info) throws TagconfException {
		if (info.paraSettings != null)
			parser.parseParaSetting(info.paraSettings);
		if (info.charSettings != null)
			parser.parseCharSetting(info.charSettings);
		if (info.freeSettings != null)
			parser.parseReplaceSetting(info.freeSettings);
	}

	private void readJavaScriptSettings(Properties prop, PropertiesInfo info)
			throws TagconfException {
		// JavaScript を実行するプロパティー名のリスト
		String[] settings = { "黒丸数字書式", "白丸数字書式", "アルファベット", "黒四角数字書式",
				"その他自由設定" };
		...
		try {
			...
			throw new TagconfException(
					"tagconf.xml の JavaScript を実行中にエラーが発生しました。\nメッセージ : "
							+ e.getMessage());
		}
	}

	public PropertiesInfo readProperties(String filePath) throws IOException,
			TagconfException {

		PropertiesInfo info = new PropertiesInfo();
		Properties prop = PropertiesLoader.getNewInstance(filePath)
				.getProperties();
		...
		try {
			...
			Logger.global.severe("指定されたエンコード'" + info.charsetName
					+ "'は不適切な値です。");
		...
		App.getInstance().getActiveParagraphTag()
				.add(new ParagraphTag(info.defaultParagraphName));
