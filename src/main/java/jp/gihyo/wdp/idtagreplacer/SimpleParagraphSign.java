...
public class SimpleParagraphSign extends ParagraphSign {
	...
	public SimpleParagraphSign(String startSign, String endSign,
			ParagraphTag tag) {
	...
	protected String whenEndSignMuches(String line) throws IOException,
		...
		if (!endSign.equals(line)) {
			line = removeSign ? line.substring(0,
					line.length() - endSign.length()) : line;
	...
	protected String whenStartSignMuches(String line) throws IOException,
		...
		if (hasEndSign()) {
			...
			// App.getInstance().getActiveParagraphTag().removeLast();
			App.getInstance().getPrintController().addCommand("print");
			App.getInstance().getPrintController()
					.addCommand("remove last para");
		}
		return line;
	}
}
