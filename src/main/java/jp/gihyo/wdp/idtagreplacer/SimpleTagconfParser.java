...
/**
 * <p>
 * タグ設定ファイル（tagconf.xml）の「段落タグ設定」や「文字タグ設定」の内容を解析するクラスです。 正規表現による単純な解析処理を行います。
 * </p>
 */
public class SimpleTagconfParser {
	...
	private String[] getStringsSplitedByColon(String line)
	...
	private String[] parseLine(String line) throws TagconfException {
		...
		case 1:
			...
			break;
		default:
			Logger.global.warning("" + signs.length
					+ "つの編集記号が見つかりました。2つ目以上の編集記号は無視されます。\n" + line);
		}
		return ret;
	}
	...
	private void setParaList(String[] items) {
		...
		if (items[0].startsWith(CALL_SCRIPT_MARK)) {
			...
			s = items.length == 3 ? new ScriptParagraphSign(items[1], items[2],
					t) : new ScriptParagraphSign(items[1], t);
		} else if (items[0].startsWith(REMAIN_SIGN_MARK)) {
			t = new ParagraphTag(items[0].substring(REMAIN_SIGN_MARK.length()));
			SimpleParagraphSign ss = items.length == 3 ? new SimpleParagraphSign(
					items[1], items[2], t) : new SimpleParagraphSign(items[1],
					t);
			ss.setRemoveSign(false);
			s = ss;
		} else {
			t = new ParagraphTag(items[0]);
			s = items.length == 3 ? new SimpleParagraphSign(items[1], items[2],
					t) : new SimpleParagraphSign(items[1], t);
		}
		App.getInstance().getParagraphSigns().add(s);
		Logger.global.info("段落スタイル'"
				+ items[0]
				+ "'には、"
				+ (items.length == 3 ? "開始タグ'" + items[1] + "'と閉じタグ'"
						+ items[2] + "'" : "開始タグ'" + items[1] + "'だけ")
				+ "が設定されました。");
	}
	...
	private void setCharList(String[] items) {
		...
		Logger.global.info("文字スタイル'" + items[0] + "'には、開始タグ'" + items[1]
				+ "'と閉じタグ'" + items[2] + "'が設定されました。");
	}
	...
	public void parseReplaceSetting(String property) {
		...
		for (String l : lines) {
			...
			App.getInstance().getReplaceSigns()
					.add(new ReplaceSign(items[0].trim(), t));
		}
